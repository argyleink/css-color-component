---
title: color-input
description: A modern web component color picker with wide-gamut support, multiple editing modes, and a gamut-aware area picker
template: splash
hero:
  title: color-input
  tagline: A wide-gamut color picker web component for modern CSS color workflows
  image:
    html: '<color-input class="hero-picker" value="oklch(70% 15% 280)"></color-input>'
  actions:
    - text: Get Started
      link: /#quick-start
      icon: right-arrow
      variant: primary
    - text: API Reference
      link: /api
      icon: external
      variant: minimal
---

import ColorInputDemo from "../../components/ColorInputDemo.astro";
import { Tabs, TabItem } from "@astrojs/starlight/components";
import { Card, CardGrid } from "@astrojs/starlight/components";

## Try it out

<ColorInputDemo value="#0bf8" />

## Limit the dropdown to the spaces you want

You can tailor the colorspace menu to a smaller supported subset with `color-spaces`.

<Tabs>
  <TabItem label="Perceptual + Hex">
    <ColorInputDemo
      value="color(display-p3 0.3 0.7 0.95)"
      colorSpaces="oklch oklab hex"
      description="Only `oklch`, `oklab`, and `hex` are shown. The first item becomes the default editing space."
    />
  </TabItem>
  <TabItem label="UI Code">
    ```html
    <color-input
      value="color(display-p3 0.3 0.7 0.95)"
      color-spaces="oklch oklab hex"
    ></color-input>
    ```
  </TabItem>
</Tabs>

If the user types a supported color in a different space, the component converts it back into the first listed space so the UI stays inside the allowed subset.

## Features

<CardGrid>
  <Card title="Wide-Gamut Colors" icon="star">
    Work with sRGB, Display P3, A98 RGB, Rec2020, ProPhoto, XYZ, and perceptual spaces like OKLCH and OKLab. The picker also detects which gamut contains the current color.
  </Card>
  <Card title="Multiple Editing Modes" icon="palette">
    Edit the same color through text input, sliders, numeric fields, a colorspace select, and a 2D area picker without losing sync.
  </Card>
  <Card title="Gamut-Aware Area Picker" icon="open-book">
    The 2D picker adapts to the active colorspace and can render gamut-aware boundaries for perceptual and wide-gamut workflows.
  </Card>
  <Card title="Customizable UI" icon="puzzle">
    Shadow DOM keeps the component self-contained while CSS parts expose the trigger, panel, input, controls, area, and readouts for styling.
  </Card>
</CardGrid>

## What it includes

`<color-input>` is more than a swatch and a few sliders. The current picker UI includes:

- an inline text field for raw CSS color strings
- numeric channel inputs paired with sliders
- a colorspace selector
- a 2D area picker that changes model per space
- a preview area with copy and EyeDropper tools
- a gamut indicator and black/white contrast readouts

These pieces stay synchronized whether the value changes through the UI, attributes, or properties.

## Quick Start

<Tabs>
  <TabItem label="Usage">
    ```html
    <script type="module">
      import 'https://cdn.jsdelivr.net/gh/argyleink/css-color-component/dist/index.js'
    </script>

    <color-input value="oklch(75% 30% 180)"></color-input>
    ```
  </TabItem>
  <TabItem label="Result">
    <ColorInputDemo
      value="oklch(75% 30% 180)"
      showCode={false}
    />
  </TabItem>
</Tabs>

:::note
The custom element auto-registers on import. Use it in any HTML document or framework. The popover UI relies on the [Popover API](https://developer.mozilla.org/en-US/docs/Web/API/Popover_API), and the EyeDropper tool appears only in browsers that support the EyeDropper API.
:::

## How the picker behaves

The component chooses a different editing plane depending on the current colorspace:

- `srgb` and `hex` use an OKHSV-style plane for practical RGB picking
- `hsl` uses saturation × lightness
- `oklch`, `lch`, and wide-gamut RGB-like spaces use a chroma × lightness plane
- `oklab` and `lab` use an `a × b` plane around the origin
- `hwb` uses whiteness × blackness

For spaces that benefit from it, the area picker can render gamut boundaries and stretch the interactive mapping so the useful region fills more of the canvas.

## Who this is for

Use it when you want a custom element that:

- works outside a framework or inside one
- accepts modern CSS color syntax
- can expose wide-gamut colors instead of collapsing everything to sRGB
- still gives users a familiar picker UI with copy, eyedropper, and keyboard support

## Next Steps

- [API Reference](/api) — Attributes, properties, methods, and events
- [Styling](/styling) — Themes, CSS parts, and customization patterns
- [Advanced](/advanced) — Architecture, gamut behavior, and performance
