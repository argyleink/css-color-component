...
```html
<color-input value="oklch(70% 20% 240)" colorspace="oklch"></color-input>
```
...
- **`hex`** - Hex editing with synchronized RGB controls and smart shorthand output
- **`srgb-linear`** - Linear-light sRGB for color-managed rendering pipelines
...
::::tip
Supplying a hex value (such as `#ff7a00`) automatically switches the component into `colorspace="hex"` so copy/paste round-trips preserve the hex format.
::::
...
```html
<button popovertarget="picker" popovertargetaction="toggle">Toggle color picker</button>
...
| `input` | Inline text field for manual color entry |
| `panel` | Popover container |
| `output` | Current color value display |
| `gamut` | Gamut badge |
| `controls` | Sliders and numeric inputs container |
