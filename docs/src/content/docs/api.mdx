...
The `<color-input>` element accepts two HTML attributes:

### value

The `value` attribute is the **only required attribute**. It sets the color using standard CSS color syntax.
...
:::note
The color space for editing is automatically detected from the value. For example, `hsl(...)` uses HSL editing, `#ff7a00` uses hex editing, etc.
:::
...
**colorspace** - Current color space (property only, not an attribute)

```js
console.log(picker.colorspace); // "oklch"
picker.colorspace = "hsl"; // Switches to HSL editing and converts the color
```

:::note
The `colorspace` property allows you to programmatically change the editing color space. Setting it will convert the current color to that space. Unlike the `value` and `theme` properties, `colorspace` does **not** reflect to an HTML attribute.
:::
...
:::note
The `value` and `theme` properties reflect to/from attributes. The `colorspace` property is available for programmatic access but does not reflect to an attribute.
:::
