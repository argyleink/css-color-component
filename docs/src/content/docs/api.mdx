...
The `no-gamut-boundaries` attribute hides the gamut boundary guide lines from the 2D area picker while leaving the underlying color editing behavior unchanged.

```html
<color-input value="color(display-p3 0.3 0.7 0.95)" no-gamut-boundaries></color-input>
```

:::tip
Use `no-gamut-boundaries` when you want a cleaner color field presentation without the extra gamut reference overlays.
:::
...
**showGamutBoundaries** - Controls gamut boundary guide visibility

```js
console.log(picker.showGamutBoundaries); // true
picker.showGamutBoundaries = false; // Hides gamut boundary lines
picker.showGamutBoundaries = true; // Shows gamut boundary lines
```
...
:::note
`value`, `colorspace`, `theme`, `noAlpha`, and `showGamutBoundaries` reflect to attributes. `gamut` and `contrastColor` are derived read-only values.
:::
