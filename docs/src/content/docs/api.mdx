...
The `<color-input>` element accepts four HTML attributes:
...
:::tip
If `value` is omitted, it defaults to `oklch(75% 75% 180)`. The component automatically converts between color spaces.
:::
...
The `<color-input>` element exposes six properties:
...
The popover panel includes a two-dimensional color area that lets users pick a color visually. The area automatically adapts its axes to the active color space:

| Active color space | Internal model | X axis | Y axis | Fixed channel |
| --- | --- | --- | --- | --- |
| sRGB, hex, HWB, Lab, OKLab | OkHSV | Saturation | Value | Hue |
| OKLCH, LCH | OKLCH | Chroma | Lightness | Hue |
| HSL | HSL | Saturation | Lightness | Hue |

### Pointer interaction

Click or drag anywhere inside the area to select a color. The thumb tracks your pointer position and the component value updates in real time.

### Keyboard navigation

Focus the area (it has `role="slider"` and is focusable) and use the arrow keys:
...
:::tip
The area gradient is rendered onto a `<canvas>` at half resolution for performance and scaled up with CSS.
:::
