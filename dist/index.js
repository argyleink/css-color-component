...
var _a, _b, _c, _area, _controller, _color, _space, _draggingColor, _effectiveConfig, _chromaLUT, _polarLUT, _worker, _renderSeqId, _lastRenderedId, _workerBusy, _pendingWorkerMsg, _d, _e, _value, _space2, _theme, _open, _anchor, _error, _noAlpha, _contrast, _gamut, _previewEffectCleanup, _colorSchemeEffectCleanup, _errorEffectCleanup, _controlsEffectCleanup, _controlsUpdate, _areaPickerEffectCleanup, _areaPicker, _programmaticUpdate, _ColorInput_instances, isInlinePickerVisible_fn, _root, _panel, _controls, _spaceSelect, _output, _chip, _internalTrigger, _textInput, _errorMessage, _lastInvoker, emitChange_fn, validateAndSetColor_fn, _lastPlacement, _lastPanelSize, _cleanup, _rafId, usingCSSAnchor_fn, startReposition_fn, stopReposition_fn, scheduleReposition_fn, positionNow_fn, getAnchorRect_fn, measurePanel_fn, renderControls_fn, updateControls_fn;
...
function fromCam16(cam16, env) {
  ...
  const p2 = A / env.nbb;
  const r2 = 23 * (p2 + 0.305) * zdiv(t2, 23 * p1 + t2 * (11 * cosh + 108 * sinh));
  const a2 = r2 * cosh;
  const b2 = r2 * sinh;
  const rgb_c = unadapt(
    /** @type {Vector3} */
...
function rgbColor(space, r2, g2, b2, a2) {
  const s2 = space === "prophoto" ? "prophoto-rgb" : space;
  return `color(${s2} ${r2}% ${g2}% ${b2}%${alphaToString(a2)})`;
}
...
function gencolor(space, ch) {
  ...
  switch (space) {
    ...
    case "hex": {
      const r2 = Math.round(Number(R) * 2.55);
      const g2 = Math.round(Number(G) * 2.55);
      const b2 = Math.round(Number(B) * 2.55);
      const a2 = Number(ALP);
      const rh = r2.toString(16).padStart(2, "0");
...
function parseIntoChannels(space, colorStr) {
  ...
  if (s2 === "oklab") {
    ...
    const [r2, g2, b2] = toGamut(c4, { space: "srgb", method: "clip" }).coords;
    ch.R = toFixed((r2 != null ? r2 : 0) * 100);
    ch.G = toFixed((g2 != null ? g2 : 0) * 100);
    ch.B = toFixed((b2 != null ? b2 : 0) * 100);
    ch.ALP = toFixed(alpha * 100);
  } else if (isRGBLike(s2)) {
    const [r2, g2, b2] = c4.coords;
    ch.R = toFixed((r2 != null ? r2 : 0) * 100);
    ch.G = toFixed((g2 != null ? g2 : 0) * 100);
    ch.B = toFixed((b2 != null ? b2 : 0) * 100);
    ch.ALP = toFixed(alpha * 100);
...
function renderAreaGradient(canvas, getColor2, dpr) {
  ...
  for (let y2 = 0; y2 < H; y2++) {
    ...
    for (let x = 0; x < W; x++) {
      const X = x / (W - 1);
      const [r2, g2, b2] = to(getColor2(X, Y), targetSpace).coords;
      const i2 = (y2 * W + x) * 4;
      d2[i2] = Math.round(Math.max(0, Math.min(1, r2 != null ? r2 : 0)) * 255);
...
function colorToPolarStretch(a2, b2, polarLUT) {
  ...
  const maxR = lerpAngleLUT(polarLUT, angle);
  const t2 = maxR > 0 ? Math.min(1, r2 / maxR) : 0;
  const edgeDist = 1 / Math.max(Math.abs(Math.cos(angle)), Math.abs(Math.sin(angle)));
    ...
    try {
      ...
      __privateGet(this, _worker).onmessage = (e2) => {
        ...
        const img = new ImageData(
          ...
          result.W,
          result.H,
          supportsP3Canvas ? { colorSpace: "display-p3" } : void 0
        );
        offCtx.putImageData(img, 0, 0);
...
const asColorSpace = (value) => COLOR_SPACES.includes(value) ? value : null;
...
class ColorInput extends HTMLElement {
  constructor() {
    ...
    __privateAdd(this, _controlsUpdate, null);
  ...
  static get observedAttributes() {
    return ["value", "colorspace", "theme", "no-alpha", "initial-colorspace"];
  }
  get value() {
  ...
  get initialColorspace() {
    return asColorSpace(this.getAttribute("initial-colorspace"));
  }
  set initialColorspace(s2) {
    const next = asColorSpace(typeof s2 === "string" ? s2 : null);
  ...
  connectedCallback() {
    ...
    if (areaPickerEl && getComputedStyle(areaPickerEl).display !== "none") {
      __privateSet(this, _areaPicker, new AreaPicker(areaPickerEl, (color, isDragging) => {
        ...
        __privateSet(this, _programmaticUpdate, false);
        __privateMethod(this, _ColorInput_instances, emitChange_fn).call(this);
        __privateMethod(this, _ColorInput_instances, updateControls_fn).call(this);
      }));
    ...
    if (btn) btn.addEventListener("click", () => {
      if (__privateMethod(this, _ColorInput_instances, isInlinePickerVisible_fn).call(this)) return;
      this.show(btn);
    });
    ...
    const resolvedInitialSpace = !this.hasAttribute("colorspace") ? asColorSpace(this.getAttribute("initial-colorspace")) : null;
    if (resolvedInitialSpace) __privateGet(this, _space2).value = resolvedInitialSpace;
...
isInlinePickerVisible_fn = function() {
  return __privateGet(this, _panel) ? getComputedStyle(__privateGet(this, _panel)).display !== "none" : false;
};
_root = new WeakMap();
_panel = new WeakMap();
...
renderControls_fn = function() {
  ...
  __privateSet(this, _controlsUpdate, (newCh) => {
    r(() => updaters.forEach((update) => update(newCh)));
  });
};
...
updateControls_fn = function() {
  if (!__privateGet(this, _controlsUpdate)) {
    __privateMethod(this, _ColorInput_instances, renderControls_fn).call(this);
    return;
  }
  try {
    const { ch } = parseIntoChannels(__privateGet(this, _space2).value, __privateGet(this, _value).value);
    __privateGet(this, _controlsUpdate).call(this, ch);
  } catch {
  }
};
